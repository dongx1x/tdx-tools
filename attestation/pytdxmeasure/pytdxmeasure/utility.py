...
class DeviceNode:
    ...
    * GET_TDQUOTE
    ...
    def qgs_msg_quote_req(self, tdreport):
        ...
        qgs_msg = struct.pack("2H5I%ds"%report_size, major_version, minor_version, msg_type,
    ...
    def qgs_msg_quote_resp(self, buf):
        ...
        major_version, minor_version, msg_type, msg_size, error_code, id_size, quote_size, quote = \
            struct.unpack("2H5I%ds"%msg_size, buf)
    ...
    def get_tdquote_bytes(self, report_data=None):
        ...
        tdreport_req = self.create_tdx_report_req(report_data)
        ...
        try:
            ...
            LOG.error("Fail to execute tdreport ioctl for file %s", self.device_node_name)
            os.close(fd_tdx_device)
            return None

        tdreport_bytes = self.get_tdreport_bytes_from_req(tdreport_req)
        ...
        try:
            ...
            buf_addr, buf_len = struct.unpack("QQ", tdquote_req)
    ...
    def create_tdx_quote_req(self, tdreport):
        ...
        if self.device_node_name == DEVICE_NODE_NAME_1_0:
            tdquote_hdr = struct.pack("QQII4s%ds"%report_size, version, status, in_len, out_len,
                            len(qgs_msg).to_bytes(4, "big"), qgs_msg)
            self.tdquote = ctypes.create_string_buffer(TDX_QUOTE_LEN)
            for index in range(len(tdquote_hdr)):
                self.tdquote[index] = tdquote_hdr[index]
    ...
    def get_tdquote_bytes_from_req(self, req):
        ...
        if self.device_node_name == DEVICE_NODE_NAME_1_0:
            ...
            data_len = buf_len - 24
            version, status, in_len, out_len, data = struct.unpack("QQII%ds"%data_len, buf)
